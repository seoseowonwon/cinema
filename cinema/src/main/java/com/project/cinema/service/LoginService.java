...
public class LoginService {
	...
	public int bringMember(LoginDto loginDto, HttpSession session){ // 아이디 비밀번호 비교 후 로그인 결정
		...
		if(loginDto.getId().equals(userInfo.get("id")) && loginDto.getPassword().equals(userInfo.get("password"))){
			return 1; // 일치하면 1을 반환
		} else {
			return 0; // 불일치시 0을 반환
		}
